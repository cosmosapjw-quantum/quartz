...
def autoscale_model_cfg(cfg, hw, alphazero_net_cls):
    ...
    filter_values = sorted(set([base_filters, _round_up_to_multiple(base_filters + 32, 32), _round_up_to_multiple(base_filters + 64, 32), filter_cap]))
    filter_values = [f for f in filter_values if base_filters <= f <= filter_cap]
    block_values = list(range(base_blocks, block_cap + 1, 2))
    vh_values = sorted(set([base_vh, _round_up_to_multiple(max(base_vh, base_filters), 64), _round_up_to_multiple(max(base_vh, base_filters * 2), 64), vh_cap]))
...
def autotune_training_cfg(cfg, hw, concurrent=True, alphazero_net_cls=None):
    ...
    tuned["batch_size"] = max(cfg.get("batch_size", 8), min(eval_batch_cap, max(tuned["n_threads"] * tuned["bg_parallel"], 8)))
    base_batch = cfg.get("batch", 256)
    batch_multiple = 32 if base_batch >= 256 else 16
    tuned["batch"] = _round_down_to_multiple(int(base_batch * train_batch_scale), batch_multiple)
    tuned["batch"] = max(batch_multiple, tuned["batch"])
    tuned["bg_batch_games"] = _autotune_batch_game_limit(hw, tuned["bg_parallel"], concurrent=True) if concurrent else 0
    if not concurrent:
...
def print_autotune_summary(original_cfg, tuned_cfg, hw):
    ...
    changed = []
    for key in ("filters", "blocks", "vh", "games", "batch", "n_threads", "batch_size", "selfplay_parallel", "bg_parallel", "bg_batch_games"):
...
def save_autotune_profile(profile_path, hw, cfg, overrides, benchmarks, hardware_signature):
    payload = {
...
def _autotune_parallel_candidates(cfg, hw, concurrent):
    ...
    if concurrent:
        seeds = [2, 3, min(4, upper), max(1, upper // 2), max(1, (upper * 2) // 3), cfg.get("bg_parallel", 1), upper]
        if hw.device_kind == "cpu" or hw.gpu_vram_mb <= 0:
            seeds.insert(0, 1)
        if upper > 4:
...
def _score_selfplay_probe(positions_per_s, cycle_s, concurrent, positions=0, eval_messages=0, model_batch_mean=0.0, parallel=1, n_threads=1):
    if not concurrent:
...
def _warmup_selfplay_probe(cfg, model, device, rust_binary, parallel, batch_games, n_threads, runtime_hooks):
    probe_cfg = dict(cfg)
    probe_cfg["n_threads"] = n_threads
    probe_cfg["batch_size"] = max(cfg.get("batch_size", 8), min(64, max(n_threads * parallel, 8)))
    ...
    try:
        runtime_hooks.selfplay_rust_nn_batched(warm_cfg, model, device, max(1, batch_games), rust_binary, parallel=parallel, show_progress=False)
    except Exception:
        pass
    _sync_device(device)


def _run_selfplay_probe(cfg, model, device, rust_binary, parallel, batch_games, n_threads, runtime_hooks, concurrent=True, rounds=1, warmup=True):
    probe_cfg = dict(cfg)
    probe_cfg["n_threads"] = n_threads
    probe_cfg["batch_size"] = max(cfg.get("batch_size", 8), min(64, max(n_threads * parallel, 8)))
    probe_cfg["_disable_resident_session"] = True
    if warmup:
        _warmup_selfplay_probe(cfg, model, device, rust_binary, parallel, batch_games, n_threads, runtime_hooks)
    n_games = max(1, batch_games * max(1, rounds))
    perf_stats = {}
    runtime_hooks.stall_trace("selfplay_probe_begin", game=cfg.get("_name"), parallel=int(parallel), batch_games=int(batch_games), n_threads=int(n_threads), rounds=int(rounds), n_games=int(n_games), iters=int(probe_cfg.get("iters", 0)), batch_size=int(probe_cfg.get("batch_size", 0)), resident=bool(probe_cfg.get("_resident_session", False)))
    _sync_device(device)
    t0 = time.time()
    states, _, _, _ = runtime_hooks.selfplay_rust_nn_batched(probe_cfg, model, device, n_games, rust_binary, parallel=parallel, show_progress=False, perf_stats=perf_stats)
    _sync_device(device)
    ...
    score = _score_selfplay_probe(positions_per_s, cycle_s, concurrent, positions=positions, eval_messages=eval_messages, model_batch_mean=model_batch_mean, parallel=parallel, n_threads=n_threads)
    runtime_hooks.stall_trace("selfplay_probe_end", game=cfg.get("_name"), parallel=int(parallel), batch_games=int(batch_games), n_threads=int(n_threads), rounds=int(rounds), positions=int(positions), elapsed_s=float(elapsed), cycle_s=float(cycle_s), positions_per_s=float(positions_per_s), eval_messages=int(eval_messages), model_calls=int(perf_stats.get("model_calls", 0) or 0))
    return {"parallel": parallel, "batch_games": batch_games, "n_threads": n_threads, "batch_size": probe_cfg["batch_size"], "probe_rounds": rounds, "positions": positions, "elapsed_s": round(elapsed, 3), "cycle_s": round(cycle_s, 3), "positions_per_cycle": round(positions_per_cycle, 3), "positions_per_s": round(positions_per_s, 3), "eval_messages": eval_messages, "eval_items": int(perf_stats.get("eval_items", 0) or 0), "model_calls": int(perf_stats.get("model_calls", 0) or 0), "model_batch_mean": round(model_batch_mean, 3), "score": round(score, 4)}


def benchmark_selfplay_throughput(cfg, model, device, rust_binary, hw, runtime_hooks, concurrent=True):
    ...
    for parallel in candidates:
        for batch_games in _autotune_batch_game_candidates(hw, parallel, concurrent):
            for n_threads in _autotune_thread_candidates(hw, parallel, hinted=cfg.get("n_threads", 1)):
                coarse_plan.append((parallel, batch_games, n_threads))
    results = []
    with runtime_hooks.tqdm_factory(len(coarse_plan), "autotune-selfplay-coarse") as pbar:
        for parallel, batch_games, n_threads in coarse_plan:
            pbar.set_postfix_str(f"p={parallel} bg={batch_games} th={n_threads}")
            try:
                probe = _run_selfplay_probe(coarse_cfg, model, device, rust_binary, parallel, batch_games, n_threads, runtime_hooks, concurrent=concurrent, rounds=1 if concurrent else 1)
            except Exception as e:
                results.append({"parallel": parallel, "batch_games": batch_games, "n_threads": n_threads, "error": str(e)})
                pbar.update(1)
                continue
            probe["stage"] = "coarse"
    ...
    with runtime_hooks.tqdm_factory(len(finalists), "autotune-selfplay-refine") as pbar:
        for row in finalists:
            pbar.set_postfix_str(f"p={row['parallel']} bg={row['batch_games']} th={row['n_threads']}")
            try:
                probe = _run_selfplay_probe(refine_cfg, model, device, rust_binary, row["parallel"], row["batch_games"], row["n_threads"], runtime_hooks, concurrent=concurrent, rounds=3 if concurrent else 1)
                probe["stage"] = "refine"
                refined.append(probe)
                results.append(probe)
            except Exception as e:
                results.append({"parallel": row["parallel"], "batch_games": row["batch_games"], "n_threads": row["n_threads"], "stage": "refine", "error": str(e)})
            pbar.update(1)
    ranking_pool = refined or scored
    best = max(ranking_pool, key=lambda r: (r["score"], r["positions_per_s"], -r["cycle_s"]))
    overrides = {"selfplay_parallel": best["parallel"], "bg_parallel": best["parallel"] if concurrent else min(best["parallel"], 4), "bg_batch_games": best["batch_games"] if concurrent else max(1, best["parallel"]), "n_threads": best["n_threads"]}
    summary = {"parallel": best["parallel"], "batch_games": best["batch_games"], "n_threads": best["n_threads"], "positions_per_cycle": best["positions_per_cycle"], "positions_per_s": best["positions_per_s"], "cycle_s": best["cycle_s"], "score": best["score"]}
    return overrides, results, summary


def benchmark_train_batch(cfg, backend, model, optimizer, device, hw, concurrent=False, target_positions_per_cycle=None):
    base_batch = cfg["batch"]
    batch_multiple = 32 if base_batch >= 256 else 16
    batch_candidates = sorted(set([_round_down_to_multiple(int(base_batch * 0.5), batch_multiple), _round_down_to_multiple(int(base_batch * 0.75), batch_multiple), base_batch, _round_down_to_multiple(int(base_batch * 1.25), batch_multiple), _round_down_to_multiple(int(base_batch * 1.5), batch_multiple)]))
    batch_candidates = [b for b in batch_candidates if b >= 32]
    if concurrent and target_positions_per_cycle:
        max_useful_batch = _round_down_to_multiple(int(max(32, target_positions_per_cycle * 4.0)), batch_multiple)
    ...
    for batch_n in batch_candidates:
        ...
        try:
            ...
            score = _score_train_batch_probe(examples_per_s, batch_n, concurrent=concurrent, target_positions_per_cycle=target_positions_per_cycle)
            results.append({"batch": batch_n, "examples_per_s": round(examples_per_s, 3), "elapsed_s": round(elapsed, 3), "score": round(score, 4)})
...
def run_autotune_benchmark(cfg, backend, model, optimizer, device, hw, rust_binary, runtime_hooks, concurrent=True):
    ...
    selfplay_meta = {}
    try:
        sp_overrides, sp_results, selfplay_meta = benchmark_selfplay_throughput(cfg, model, device, rust_binary, hw, runtime_hooks, concurrent=concurrent)
        overrides.update(sp_overrides)
        benchmark["selfplay"] = sp_results
    except Exception as e:
        benchmark["selfplay"] = [{"error": str(e)}]
    try:
        train_overrides, train_results = benchmark_train_batch(apply_runtime_overrides(cfg, overrides), backend, model, optimizer, device, hw, concurrent=concurrent, target_positions_per_cycle=selfplay_meta.get("positions_per_cycle"))
    ...
    if "bg_parallel" in overrides and "batch_size" not in overrides:
        overrides["batch_size"] = max(cfg.get("batch_size", 8), min(64, max(overrides["bg_parallel"] * overrides.get("n_threads", cfg.get("n_threads", 1)), 4)))
    return overrides, benchmark
...
def plan_online_runtime_overrides(cfg, hw, sample):
    ...
    if hw.device_kind != "cpu" and hw.gpu_vram_mb > 0 and n_threads == 1 and parallel >= max(4, max_parallel) and thread_capacity >= 2:
    ...
    if max(last_cycle_positions, int(sample.get("rolling_positions", 0) or 0)) > 0:
        ...
        target_batch = _round_down_to_multiple(int(max(64, effective_positions * 3.5)), batch_multiple)
