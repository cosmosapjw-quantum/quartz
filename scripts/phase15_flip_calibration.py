...
def bundle_decision_records(
    ...
    policies = [np.asarray(p, dtype=np.float64) for p in bundle.get("trace_policies", [])]
...
def reliability_diagram(preds: Sequence[float], ys: Sequence[int], *, n_bins: int = 10) -> dict[str, Any]:
    ...
    for k in range(n_bins):
        ...
        cnt = int(mask.sum())
        if cnt == 0:
            bins.append({"lo": float(lo), "hi": float(hi), "count": 0, "mean_pred": None, "mean_obs": None})
            continue
        mean_pred = float(p[mask].mean())
        mean_obs = float(y[mask].mean())
        bins.append({"lo": float(lo), "hi": float(hi), "count": cnt, "mean_pred": mean_pred, "mean_obs": mean_obs})
        ece += (cnt / p.size) * abs(mean_obs - mean_pred)
    brier = float(np.mean((p - y) ** 2))
    return {"n": int(p.size), "bins": bins, "ece": float(ece), "brier": brier}


def virtual_stop_budget(records: Sequence[dict[str, Any]], predictor_key: str, threshold: float) -> dict[str, Any]:
    ...
    for r in usable:
        if float(r[predictor_key]) >= threshold:
            return {"stop_budget": int(r["budget"]), "agreement": int(r["y"]), "fired": True}
    last = usable[-1]
    return {"stop_budget": int(last["budget"]), "agreement": int(last["y"]), "fired": False}
...
def matched_budget_calibration(
    ...
    *,
    ...
    h1_stops = [virtual_stop_budget(recs, "s_h1", h1_threshold) for recs in bundle_records]
    h1_valid = [(recs, s) for recs, s in zip(bundle_records, h1_stops) if s["stop_budget"] is not None]
    if not h1_valid:
    ...
    for tau in pflip_threshold_grid:
        ...
        paired = [(h1["agreement"], pf["agreement"]) for (_, h1), pf in zip(h1_valid, pf_stops)
                  if pf["stop_budget"] is not None and pf["agreement"] is not None and h1["agreement"] is not None]
        if not paired:
            continue
        pf_mean_budget = float(np.mean([pf["stop_budget"] for pf in pf_stops if pf["stop_budget"] is not None]))
        rel = abs(pf_mean_budget - h1_mean_budget) / max(1.0, h1_mean_budget)
        cand = {"pflip_threshold": float(tau), "pflip_mean_budget": pf_mean_budget, "budget_rel_gap": rel, "n_paired": len(paired), "paired": paired}
        if rel <= match_tolerance and (best is None or rel < best["budget_rel_gap"]):
            best = cand
    if best is None:
        return {"insufficient": True, "reason": "no pflip threshold matched H1 budget within tolerance", "h1_mean_budget": h1_mean_budget}
...
def analyze(bundles: Sequence[dict[str, Any]], *, alpha: float = 0.5, n_boot: int = 4000, seed: int = 0,
            h1_threshold: float = 0.9) -> dict[str, Any]:
    per_bundle = [bundle_decision_records(b, alpha=alpha, n_boot=n_boot, seed=seed) for b in bundles]
    ...
    pf_rel = reliability_diagram([r["s_pflip"] for r in pf_records], [r["y"] for r in pf_records])
    matched = matched_budget_calibration(per_bundle, h1_threshold=h1_threshold, seed=seed)
    return {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--trace-dir", required=True, help="phase15 trace-cache dir (schema>=5 bundles)")
    p.add_argument("--h1-threshold", type=float, default=0.9)
    p.add_argument("--n-boot", type=int, default=4000)
    p.add_argument("--alpha", type=float, default=0.5)
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--output", default="results/phase15_stage7/flip_calibration/summary.json")
    return p
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = analyze(bundles, alpha=args.alpha, n_boot=args.n_boot, seed=args.seed, h1_threshold=args.h1_threshold)
