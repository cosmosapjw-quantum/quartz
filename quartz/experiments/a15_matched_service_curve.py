...
from quartz.host_resources import validate_host_resource_contract
...
def validate_config(payload: Mapping[str, Any], profile_name: str) -> dict[str, Any]:
    ...
    host_resources_raw = payload.get("host_resource_contract")
    if not isinstance(host_resources_raw, dict):
        raise ValueError("host_resource_contract must be an object")
    host_resources = validate_host_resource_contract(host_resources_raw)
    for key in (
        "require_guard_for_profiles",
        "require_gpu_monitor_for_profiles",
    ):
        if not all(item in profiles for item in host_resources[key]):
            raise ValueError(
                f"host_resource_contract.{key} must contain known profiles"
            )
