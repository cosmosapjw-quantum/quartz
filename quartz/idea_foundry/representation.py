...
class RwRestLiteSpec:
    ...
    def validate(self) -> None:
        ...
        if not self.static_prune_for_inference:
            raise ValueError("per-position hard graphs are incompatible with efficient MCTS batching")
...
class RegretStateArchiveSkeleton:
    ...
    def priority(self, record: ArchiveRecord) -> float:
        def nz(value: float | None) -> float:
            return 0.0 if value is None or not np.isfinite(value) else max(0.0, float(value))
    ...
    def normalized_priorities(self, records: Sequence[ArchiveRecord]) -> np.ndarray:
        ...
        scores = np.asarray([self.priority(record) for record in records], dtype=np.float64)
