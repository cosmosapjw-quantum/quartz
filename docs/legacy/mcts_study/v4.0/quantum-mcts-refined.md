...
**Implementation Update**: This document reflects critical corrections to ensure mathematical consistency: (1) discrete information time evolution $\delta\tau_N = 1/(N_{\text{root}} + 2)$, (2) Hamiltonian derived via proper Legendre transformation, and (3) causality-preserving operator evaluation with pre-update visit counts.
