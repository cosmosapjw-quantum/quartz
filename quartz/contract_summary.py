...
def summarize_contract_collection(
    ...
    summary = {
        ...
        "legacy_partial_count": sum(
            1 for row in contracts if row.get("legacy_partial")
        ),
    }
...
def summarize_named_contract_map(
    contract_map: dict | None, discarded: list[dict] | None = None, *, name_key: str
) -> dict:
    rows = [
...
def summarize_plain_contracts(contracts: list[dict] | None) -> dict:
    return summarize_contract_collection(
        list(contracts or []), [], hash_key="stable_json_hash"
    )
