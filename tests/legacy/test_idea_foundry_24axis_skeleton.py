...
from quartz.idea_foundry.legacy_24axis.analysis import (
...
from quartz.idea_foundry.legacy_24axis.contracts import (
    ActionEvidence,
    RootSnapshot,
    RuntimeEvidence,
)
from quartz.idea_foundry.legacy_24axis.decision import (
    StaticAnchorRpoSkeleton,
    StopCouncilSkeleton,
)
from quartz.idea_foundry.legacy_24axis.registry import (
    AXIS_SPECS,
    axis_ids,
    get_axis_spec,
)
from quartz.idea_foundry.legacy_24axis.representation import (
    CpuIncrementalStudentSpec,
    DiffusionRegularizedEvaluatorSpec,
    RegretStateArchiveSkeleton,
    RwRestLiteSpec,
)
from quartz.idea_foundry.legacy_24axis.systems import (
    ServiceCurvePoint,
...
def test_legacy_catalog_is_namespaced_and_noncontrolling():
    current = json.loads(
        (REPO_ROOT / "configs" / "idea_foundry.axes.v1.json").read_text(
            encoding="utf-8"
        )
    )

    assert [axis["id"] for axis in current["axes"]] == [
        f"A{index:02d}" for index in range(1, 27)
    ]
    assert len(AXIS_SPECS) == 24
    assert importlib.util.find_spec("quartz.idea_foundry.registry") is None
