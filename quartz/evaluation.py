...
class Glicko2:
    @staticmethod
    def to_g2(mu_elo, rd_elo, P):
        return (mu_elo - P.mu0_elo) / P.scale, rd_elo / P.scale

    @staticmethod
    def to_elo(mu, phi, P):
        return mu * P.scale + P.mu0_elo, phi * P.scale
    ...
    def _update_from_accumulators(mu, phi, sigma, s_g2E, s_gsE, tau):
        ...
        def f(x):
            ex = math.exp(x)
            return (
                ex * (delta**2 - phi**2 - v - ex) / (2.0 * (phi**2 + v + ex) ** 2)
            ) - (x - a) / (tau**2)
...
class GameSpec:
    ...
    board_size: int = 0
    rules_hash: str = ""
    search_budget_type: str = "simulations"
    search_budget: int = 800
...
class AnchorManifest:
            ...
            json.dump(
                {
                    ...
                    "game_spec": self.game_spec.__dict__ if self.game_spec else {},
    ...
    def load(cls, path: Path) -> "AnchorManifest":
        ...
        return cls(
            ...
            game_spec=gs,
            anchors=[AnchorEntry(**a) for a in d.get("anchors", [])],
    ...
    def default_gomoku7(cls):
        return cls(
            game_spec=GameSpec(game_type="gomoku", board_size=7, search_budget=800),
            anchors=[
                AnchorEntry(
                    id="random",
                    engine_hash="random_v1",
                    target_elo=0.0,
                    weight=1.0,
                ...
                AnchorEntry(
                    id="search_only",
                    engine_hash="uct_uniform_128_v1",
                    target_elo=200.0,
                    weight=1.0,
                    description="UCT + uniform prior, 128 sims",
                ),
            ],
        )
...
class ScaleCalibrator:
    ...
    def calibrate(self, ladder, min_expected=0.05, max_expected=0.95):
        ...
        for anchor in self.manifest.anchors:
            if anchor.id not in ladder.players:
                continue
            rec = ladder.get(anchor.id)
            if rec.games_played == 0:
                continue
            if rec.rd_elo > ladder.P.rd0_elo * 0.95:
                continue
        ...
        if len(points) < 1:
            ...
            Sr = sum(w * r for r, _, w in points) / W
            St = sum(w * t for _, t, w in points) / W
            Srr = sum(w * r * r for r, _, w in points) / W
            Srt = sum(w * r * t for r, t, w in points) / W
            denom = Srr - Sr * Sr
            if abs(denom) > 1e-10:
                self.a = (Srt - Sr * St) / denom
                self.b = St - self.a * Sr
                if self.a <= 0 or self.a > 3.0 or self.a < 0.3:
                    self.a = 1.0
                    self.b = St - Sr
            else:
                self.a = 1.0
                self.b = St - Sr
        self._history.append((self.a, self.b))
        return self.a, self.b

    def published_elo(self, internal_elo):
        return self.a * internal_elo + self.b

    def residuals(self, ladder):
        return {
            a.id: round(self.published_elo(ladder.get(a.id).mu_elo) - a.target_elo, 1)
            for a in self.manifest.anchors
            if a.id in ladder.players
        }
...
class RatingLadder:
    ...
    def _recenter(self):
        rand = self._ensure("random")
        off = self.P.anchor_target_elo - rand.mu_elo
        if abs(off) < 1e-12:
            return
        for r in self.players.values():
            r.mu_elo += off
            if r.published_elo is not None:
                r.published_elo += off
    ...
    def record_match(self, player, opponent, wins, draws, losses):
        ...
        if total <= 0:
            return
        P = self.P
        A, B = self.get(player), self.get(opponent)
        for cw, cd, cl in self._split_match_counts(wins, draws, losses):
            chunk_total = cw + cd + cl
            muA, phiA = Glicko2.to_g2(A.mu_elo, A.rd_elo, P)
            muB, phiB = Glicko2.to_g2(B.mu_elo, B.rd_elo, P)
            gB, EB = Glicko2.g(phiB), Glicko2.E(muA, muB, phiB)
            gA, EA = Glicko2.g(phiA), Glicko2.E(muB, muA, phiA)
            ...
            mu2, phi2, sig2_b = Glicko2.update_weighted(
                muB, phiB, B.sigma, termsB, P.tau
            )
            ...
            A.games_played += chunk_total
            B.games_played += chunk_total
            A.last_period = self._period
            B.last_period = self._period
        A.history.append(A.mu_elo)
        B.history.append(B.mu_elo)
        self._recenter()
    ...
    def update_from_period(self, matches):
        ...
        for p, o, w, d, l in matches:
            for name in (p, o):
                if name not in pre:
                    rec = self.get(name)
                    mu, phi = Glicko2.to_g2(rec.mu_elo, rec.rd_elo, P)
        ...
        for p, o, w, d, l in matches:
            ...
            mu_o, phi_o, _ = pre[o]
            gO = Glicko2.g(phi_o)
            EO = Glicko2.E(mu_p, mu_o, phi_o)
            terms_p = player_terms.setdefault(p, [])
            if w:
                terms_p.append((gO, EO, 1.0, w))
            if d:
                terms_p.append((gO, EO, 0.5, d))
            if l:
                terms_p.append((gO, EO, 0.0, l))
            gP = Glicko2.g(phi_p)
            EP = Glicko2.E(mu_o, mu_p, phi_p)
            terms_o = player_terms.setdefault(o, [])
            if l:
                terms_o.append((gP, EP, 1.0, l))
            if d:
                terms_o.append((gP, EP, 0.5, d))
            if w:
                terms_o.append((gP, EP, 0.0, w))
            player_games[p] = player_games.get(p, 0) + total
            player_games[o] = player_games.get(o, 0) + total
        for name, terms in player_terms.items():
            ...
            rec = self.get(name)
            rec.mu_elo, rec.rd_elo = Glicko2.to_elo(mu2, phi2, P)
            rec.sigma = sig2
            rec.games_played += player_games.get(name, 0)
            rec.last_period = self._period
            rec.history.append(rec.mu_elo)
        self._recenter()
    ...
    def save(self, path):
        data = {
            ...
            for name, r in self.players.items()
        }
    ...
    def load(self, path):
        with open(path) as f:
            data = json.load(f)
        meta = data.pop("_meta", {})
        self._period = meta.get("period", 0)
        self._bootstrapped = True
        for name, d in data.items():
            rec = RatingRecord(
                ...
                last_period=d.get("last_period", 0),
...
def score_rate_ci(wins, draws, total, z=1.96):
    if total <= 0:
        return 0.0, (0.0, 0.0)
    sr = (wins + 0.5 * draws) / total
    losses = total - wins - draws
    var = (
        wins * (1 - sr) ** 2 + draws * (0.5 - sr) ** 2 + losses * (0 - sr) ** 2
    ) / total
...
def binomial_p_value(wins, total, p0=0.5):
    ...
    var = total * p0 * (1 - p0)
    if var <= 0:
        return 1.0 if wins == total * p0 else 0.0
...
class GameRecord:
    ...
    def to_jsonl(self):
        d = {
            ...
            "engines": [self.engine_black, self.engine_white],
...
class MatchRunner:
    ...
    def play_game(
        self, eng_black, eng_white, game_id, opening_idx=None, collect_moves=True
    ):
        game = self.game_factory()
        eng_black.reset()
        eng_white.reset()
        engines = {0: eng_black, 1: eng_white}
        moves = []
        opening_applied = []
        ...
        if opening_idx is not None and opening_idx < len(self.opening_book):
            for a in self.opening_book[opening_idx]:
                if game.is_terminal() or a not in game.legal_moves():
                    break
                game.apply_move(a)
                opening_applied.append(a)
        t0 = time.time()
        ply = len(opening_applied)
        try:
            while not game.is_terminal() and ply < self.max_moves:
                p = game.current_player()
                action, meta = engines[p].select_move(game)
                ...
                if collect_moves:
                    moves.append(
                        MoveRecord(
                            ...
                            player=p,
                            action=action,
                            time_ms=meta.get("time_used_ms", 0),
                            sims=meta.get("simulations", 0),
    ...
    def play_match(self, eng_a, eng_b, num_games, color_swap=True):
        ...
        for i in range(pairs):
            ...
            records.append(self.play_game(eng_a, eng_b, f"g{idx:04d}", oi))
            idx += 1
            records.append(self.play_game(eng_b, eng_a, f"g{idx:04d}", oi))
            idx += 1
    ...
    def play_match_tally_range(
        ...
        for i in range(pair_start, pair_start + pair_count):
            ...
            if logger is not None:
                logger.log(rec)
            rec = self.play_game(
                eng_b, eng_a, f"g{idx + 1:04d}", oi, collect_moves=False
            )
            _accumulate_tally_record(tally, rec, engine_name)
            if logger is not None:
                logger.log(rec)
    ...
    def play_match_tally(self, eng_a, eng_b, num_games, color_swap=True, logger=None):
        ...
        return self.play_match_tally_range(
            eng_a, eng_b, 0, pairs, single_start, single_count, logger=logger
        )
    ...
    def play_match_tally_batched(
        ...
        if not hasattr(eng_a, "select_moves_batch") or not hasattr(
            eng_b, "select_moves_batch"
        ):
            raise TypeError(
                "batched tally requires engines with select_moves_batch(states)"
            )
        ...
        while True:
            ...
            for engine_key, batch in grouped.items():
                ...
                if len(results) != len(batch):
                    for sess in batch:
                        sess["error"] = (
                            f"batch result length mismatch: expected {len(batch)} got {len(results)}"
                        )
                        sess["done"] = True
                    continue
                for sess, (action, meta) in zip(batch, results):
                    ...
                    sess["total_time_ms"] += (
                        move_time_ms if move_time_ms > 0.0 else share_ms
                    )
                    applied = False
                    if hasattr(game, "apply_engine_meta"):
                        applied = bool(game.apply_engine_meta(action, meta))
...
class MatchTally:
    ...
    def score_rate(self):
        n = self.scored
        return (self.wins + 0.5 * self.draws) / n if n > 0 else 0.0
...
class PromotionResult:
    ...
    def to_dict(self):
        return {
            ...
            "games_scored": self.games_scored,
            "reason": self.reason,
        }
...
class PromotionGate:
    ...
    def evaluate(self, tally):
        n = tally.scored
        z = Z_TABLE.get(self.cfg.confidence, 1.96)
        sr, sr_ci = score_rate_ci(tally.wins, tally.draws, n, z)
        scored = tally.wins + 0.5 * tally.draws
        wl = wilson_ci(scored, n, z)[0]
        pv = binomial_p_value(scored, n, 0.5)
        sig = n >= self.cfg.min_games and pv < (1 - self.cfg.confidence)
        if n < self.cfg.min_games:
            verdict = PromotionVerdict.NEED_MORE
            reason = f"Only {n}/{self.cfg.min_games} scored games"
        elif sr >= self.cfg.threshold:
            if self.cfg.require_significance and not sig:
                verdict = PromotionVerdict.NEED_MORE
                reason = f"sr={sr:.3f}≥{self.cfg.threshold} but p={pv:.4f} not sig"
            else:
                verdict = PromotionVerdict.PROMOTE
                reason = f"sr={sr:.3f}≥{self.cfg.threshold}"
        else:
...
class SanityCheck:
    ...
    def check(self, candidate, game_factory, seed=None):
        ...
        tally = runner.play_match_tally(
            candidate, rand_eng, self.num_games, color_swap=True
        )
        sr = tally.score_rate
        ok = sr >= self.min_sr
        reason = f"sr vs random={sr:.3f} {'≥' if ok else '<'} {self.min_sr}"
        return SanityResult(
            passed=ok, win_rate=sr, games_played=tally.scored, reason=reason
        )
...
class ChampionTracker:
    ...
    def load(cls, path, bridge_size=3):
        with open(path) as f:
            data = json.load(f)
        tracker = cls(
            initial_id=data.get("champion", "gen_0"),
            save_path=path,
            bridge_size=bridge_size,
        )
        tracker.champion = ChampionState(
            model_id=data.get("champion", "gen_0"),
    ...
    def try_promote(self, candidate_id, generation, result, published_elo=None):
        if result.verdict != PromotionVerdict.PROMOTE:
            return False
        old_id = self.champion.model_id
        if old_id not in self.bridge:
            self.bridge.append(old_id)
        while len(self.bridge) > self.bridge_size:
            self.bridge.pop(0)
        entry = {
            "from": old_id,
            "to": candidate_id,
            ...
            json.dump(
                {
                    ...
                    "elo": self.champion.elo,
                    "history": self.champion.promotion_history,
                    "bridge": self.bridge,
                },
                f,
                indent=2,
...
class EvalConfig:
    ...
    confidence: float = 0.95
    sanity_check_interval: int = 5
    sanity_min_score: float = 0.90
    sanity_games: int = 20
    anchor_match_games: int = 20
    bridge_match_interval: int = 10
    bridge_size: int = 3
...
class TrainingEvaluator:
    def __init__(self, config=None, manifest=None):
        ...
        self.gate = PromotionGate(
            PromotionConfig(
                threshold=self.cfg.promotion_threshold,
                min_games=self.cfg.num_games,
                confidence=self.cfg.confidence,
            )
        )
        self.sanity = SanityCheck(
            min_score_rate=self.cfg.sanity_min_score, num_games=self.cfg.sanity_games
        )
        ...
        if self.cfg.champion_path and Path(self.cfg.champion_path).exists():
            self.champion = ChampionTracker.load(
                Path(self.cfg.champion_path), bridge_size=self.cfg.bridge_size
            )
        else:
            self.champion = ChampionTracker(
                save_path=Path(self.cfg.champion_path)
                if self.cfg.champion_path
                else None,
    ...
    def _run_parallel_chunk(
        ...
        runner = MatchRunner(
            game_factory, opening_book, self.cfg.seed, self.cfg.max_moves
        )
        lines = []
        logger = None
        if collect_logs:
            ...
            class _ListLogger:
                ...
                def log(self, rec):
                    self.lines.append(rec.to_jsonl())
    ...
    def evaluate_checkpoint(
        ...
        use_batched = hasattr(candidate, "select_moves_batch") and hasattr(
            champion, "select_moves_batch"
        )
        use_parallel = (
            cfg.parallel_workers > 1
            and candidate_factory is not None
            and champion_factory is not None
        )
        if use_batched:
            ...
            if cfg.log_path:
                ...
                tally = runner.play_match_tally_batched(
                    candidate, champion, cfg.num_games, cfg.color_swap
                )
        elif use_parallel:
            chunks = self._build_match_chunks(
                cfg.num_games, cfg.color_swap, cfg.parallel_workers
            )
        ...
        result.tally = {
            ...
            "total": tally.total,
            "scored": tally.scored,
            "score_rate": round(tally.score_rate, 4),
        }
        result.promotion = promo.to_dict()
        if tally.scored <= 0:
            result.valid_eval = False
            result.invalid_reason = f"zero scored games (errors={tally.errors}, voids={tally.voids}, total={tally.total})"
        ...
        self.ladder.record_match(
            candidate_key, champion_key, tally.wins, tally.draws, tally.losses
        )
        cand_rec = self.ladder.get(candidate_key)
        champ_rec = self.ladder.get(champion_key)
        ...
        result.elo = {
            "candidate": round(cand_rec.mu_elo, 1),
            "champion": round(champ_rec.mu_elo, 1),
            "delta": round(match_delta, 1),
            "ladder_delta": round(ladder_delta, 1),
        }
        result.published = {
            "candidate_abs": cand_pub,
            "candidate_abs_raw": cand_pub_raw,
            "champion_abs": champ_pub,
            "delta": round(cand_pub - champ_pub, 1),
            "scale_a": round(a, 4),
            "offset_b": round(b, 1),
        }
        if promo.verdict == PromotionVerdict.PROMOTE:
            self.champion.try_promote(
                candidate_key, generation, promo, published_elo=cand_pub
            )
        else:
            self.champion.champion.elo = champ_pub
            if self.champion.save_path:
                self.champion._save()
        if cfg.ladder_path:
            self.ladder.save(Path(cfg.ladder_path))
        result.duration_s = time.time() - t0
        return result
...
def _run_all():
    ...
    assert abs(Glicko2.g(1.7269) - 0.7242) < 0.002
    assert abs(mu2 - (-0.2069)) < 0.02
    assert abs(phi2 - 0.8722) < 0.02
    assert abs(sig2 - 0.05999) < 0.001
    print("[PASS] Glicko-2 math")
    # Promotion
    gate = PromotionGate(
        PromotionConfig(threshold=0.55, min_games=10, require_significance=False)
    )
    t = MatchTally("c", "b", wins=7, draws=2, losses=1)
    r = gate.evaluate(t)
    assert r.verdict == PromotionVerdict.PROMOTE
    t2 = MatchTally("c", "b", wins=4, draws=2, losses=4)
    r2 = gate.evaluate(t2)
    ...
    promote = PromotionResult(
        PromotionVerdict.PROMOTE, 0.65, (0.55, 0.75), 0.55, 0.01, True, 200, "ok"
    )
    ct.try_promote("gen_5", 5, promote)
    assert ct.champion.model_id == "gen_5"
    ct.try_promote("gen_10", 10, promote)
    assert ct.bridge == ["gen_0", "gen_5"]
    ct.try_promote("gen_15", 15, promote)
    assert ct.bridge == ["gen_5", "gen_10"]
    print("[PASS] Champion tracker + bridge")
    # Wilson CI
    sr, ci = score_rate_ci(40, 20, 100)
    assert abs(sr - 0.50) < 0.001
    print("[PASS] Score rate CI")
    # RD inflation
    ladder = RatingLadder()
    ladder.record_match("m", "random", 10, 0, 0)
    rd0 = ladder.get("m").rd_elo
    ladder.advance_period()
    assert ladder.get("m").rd_elo > rd0
    print("[PASS] RD inflation")
    # Batch vs sequential
    ls = RatingLadder()
    ls.record_match("A", "B", 8, 1, 1)
    ls.record_match("A", "C", 1, 1, 8)
    lb = RatingLadder()
    lb.update_from_period([("A", "B", 8, 1, 1), ("A", "C", 1, 1, 8)])
    assert abs(ls.get("A").mu_elo - lb.get("A").mu_elo) > 0.5
    print("[PASS] Batch vs sequential")
    # Scale calibrator
    mf = AnchorManifest(
        anchors=[
            AnchorEntry(id="A0", engine_hash="r", target_elo=0.0),
            AnchorEntry(id="A1", engine_hash="u", target_elo=400.0),
        ]
    )
    ld = RatingLadder()
    ld.record_match("A0", "A1", 2, 1, 7)
    cal = ScaleCalibrator(mf)
    a, b = cal.calibrate(ld)
