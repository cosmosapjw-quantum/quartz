...
class UncertaintyDecompositionSkeleton:
    ...
    def propose(self, snapshot: RootSnapshot) -> Sequence[MetaProposal]:
        mean_radius = float(np.mean([a.total_radius for a in snapshot.visible_actions])) if snapshot.visible_actions else 0.0
        return [
            MetaProposal(
...
class StopCouncilSkeleton:
    ...
    def estimated_wrong_risk(self, snapshot: RootSnapshot) -> float:
        signals: list[float] = [float(np.clip(snapshot.candidate_omission_risk, 0.0, 1.0))]
    ...
    def interval_certificate(snapshot: RootSnapshot) -> tuple[bool, float]:
        ...
        runner_upper = max((a.upper for a in snapshot.visible_actions if a.action_id != best.action_id), default=float("-inf"))
...
class StaticAnchorRpoSkeleton:
    ...
    def improved_policy(self, actions: Sequence[ActionEvidence]) -> dict[int, float]:
        if not actions:
            return {}
        anchor = normalize_prob([max(a.prior_anchor, self.prior_floor) for a in actions])
        ...
        scores = np.asarray(
            [a.lower if self.use_lower_bound else a.mean_q for a in actions if a.visible],
            dtype=np.float64,
        )
        logits = np.log(np.clip(visible_anchor, self.prior_floor, 1.0)) + scores / max(self.temperature, 1e-6)
    ...
    def propose(self, snapshot: RootSnapshot) -> Sequence[MetaProposal]:
        ...
        return [
            MetaProposal(
                ...
                telemetry={"policy": policy, "temperature": self.temperature, "uses_lower_bound": self.use_lower_bound},
            )
        ]
...
class KgVoiAllocatorSkeleton:
    ...
    def propose(self, snapshot: RootSnapshot) -> Sequence[MetaProposal]:
        ...
        if best is None:
            return []
        candidates = [a for a in snapshot.visible_actions if a.action_id != best.action_id]
        if not candidates:
            return []
        scored = []
        ...
        _, gain, target = max(scored, key=lambda item: (item[0], item[2].upper, -item[2].edge_pos))
        return [
            MetaProposal(
                axis_id=self.axis_id,
                ...
                regret_reduction_lcb=0.0,
                cost=MetaCost(nn_evals=float(self.batch_amount), cpu_ms=target.cost_ms * self.batch_amount),
                confidence=0.0,
...
class PriorRefreshSpecialistSkeleton:
    ...
    def propose(self, snapshot: RootSnapshot) -> Sequence[MetaProposal]:
        eval_unc = max((a.epistemic_radius + a.bias_radius for a in snapshot.visible_actions), default=0.0)
        if snapshot.prior_visit_js < self.min_prior_visit_js or eval_unc < self.min_eval_uncertainty:
            return []
        return [
            MetaProposal(
                axis_id=self.axis_id,
                ...
                telemetry={"prior_visit_js": snapshot.prior_visit_js, "eval_uncertainty": eval_unc},
            )
        ]
...
class MentsDecayingEntropySkeleton:
    ...
    def temperature(self, root_visits: int) -> float:
        return self.initial_temperature * math.exp(-math.log(2.0) * root_visits / max(self.half_life_visits, 1e-6))
