...
def _records(n, burst_hard_rate, nonburst_hard_rate, burst_frac, seed=0):
    ...
    for i in range(n):
        ...
        recs.append({"checkpoint_id": "C1", "position_id": f"P{i}", "burst": burst, "hard": hard})
    return recs
...
def test_o6_lift_alive_when_burst_tracks_difficulty():
    ...
    recs = _records(400, burst_hard_rate=0.8, nonburst_hard_rate=0.15, burst_frac=0.3, seed=1)
...
def test_o6_kill_when_burst_fires_at_base_rate():
    ...
    recs = _records(400, burst_hard_rate=0.3, nonburst_hard_rate=0.3, burst_frac=0.3, seed=2)
    res = m.compute_o6_lift(recs, n_boot=500, seed=0)
...
def test_o6_degenerate_when_too_few_burst_events():
    m = _load()
    recs = _records(400, burst_hard_rate=0.9, nonburst_hard_rate=0.2, burst_frac=0.03, seed=3)
...
def test_o6_degenerate_when_burst_rate_saturated():
    m = _load()
    recs = _records(200, burst_hard_rate=0.5, nonburst_hard_rate=0.5, burst_frac=0.98, seed=4)
...
def test_build_records_join_and_missing_bundle_excluded():
    ...
    online_rows = [
        ...
        {"checkpoint_id": "C1", "position_id": "P1", "budget_burst_triggered": 0},  # OR => burst 1
        {"checkpoint_id": "C1", "position_id": "P2", "budget_burst_triggered": 0},
        {"checkpoint_id": "C1", "position_id": "P3", "budget_burst_triggered": 1},  # bundle missing
    ]
    # P1: late-flip bundle (overturns => hard 1); P2: stable (hard 0). P3 has no bundle.
    bundles = {
        ("C1", "P1"): {"trace_budgets": [8, 16, 32], "trace_policies": [[0.45, 0.44, 0.11], [0.4, 0.5, 0.1], [0.3, 0.62, 0.08]]},
        ("C1", "P2"): {"trace_budgets": [8, 16, 32], "trace_policies": [[0.7, 0.2, 0.1], [0.7, 0.2, 0.1], [0.7, 0.2, 0.1]]},
    }
