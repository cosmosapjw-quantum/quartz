    ...
    def propose(self, snapshot: RootSnapshot) -> Sequence[MetaProposal]:
        return [
            MetaProposal(
                ...
                telemetry={"readout_policy": self.readout(snapshot), "curvature": self.curvature},
            )
        ]
...
class CoherenceSignedPathShadowSkeleton:
    ...
    def coherence(self, snapshot: RootSnapshot) -> float:
        stability = 0.0 if snapshot.h1_stability is None else float(np.clip(snapshot.h1_stability, 0.0, 1.0))
        return float(math.exp(-snapshot.root_visits / max(self.decay_visits, 1e-6)) * (1.0 - stability))
    ...
    def feature(self, snapshot: RootSnapshot) -> Mapping[int, float]:
        ...
        for action in snapshot.visible_actions:
            ...
            magnitude = min(self.clip, max(0.0, action.total_radius + abs(action.mean_q)))
            vector = np.asarray([magnitude * math.cos(phase), magnitude * math.sin(phase)])
    ...
    def propose(self, snapshot: RootSnapshot) -> Sequence[MetaProposal]:
        return [
            MetaProposal(
                ...
                telemetry={"coherence": self.coherence(snapshot), "feature": self.feature(snapshot)},
            )
        ]
...
class PhysicsFalsifierSkeleton:
    ...
    def fit_effective_beta(self, policy: Sequence[float], scores: Sequence[float]) -> Mapping[str, float]:
        ...
        for beta in self.beta_grid:
            ...
            kl = float(np.sum(p * (np.log(np.clip(p, 1e-12, 1.0)) - np.log(np.clip(model, 1e-12, 1.0)))))
            if kl < best_kl:
                best_kl = kl
    ...
    @staticmethod
    def redundancy_curve(fragment_decisions: Sequence[int], full_decision: int) -> Mapping[str, float]:
        if not fragment_decisions:
            return {"n_fragments": 0.0, "agreement": 0.0}
        agreement = sum(int(x == full_decision) for x in fragment_decisions) / len(fragment_decisions)
        return {"n_fragments": float(len(fragment_decisions)), "agreement": float(agreement)}
...
class SymmetryOrbitAuditSkeleton:
    ...
    def audit_policy(self, original: Sequence[float], transformed: Sequence[float], inverse_permutation: Sequence[int]) -> Mapping[str, float | bool]:
