        ...
        for action in snapshot.actions:
            ...
            if action.visible:
                visible_terms.append(
                    prior * math.exp(np.clip(action.mean_q / tau, -80.0, 80.0))
                )
            else:
    ...
    def propose(self, snapshot: RootSnapshot) -> Sequence[MetaProposal]:
        ...
        return [
            MetaProposal(
                ...
                telemetry={
                    "z_live": z_live,
                    "z_out_upper": z_out_upper,
                    "residual_ratio_upper": ratio,
                },
            )
        ]
...
class JlbRootSamplerSkeleton:
    ...
    def transition_matrix(
        self, actions: Sequence[ActionEvidence]
        ...
        for i in range(n):
            for j in range(n):
                if i == j:
                    continue
                rates[i, j] = kernel[i, j] * math.sqrt(
                    max(target[j], 1e-12) / max(target[i], 1e-12)
                )
    ...
    def propose(self, snapshot: RootSnapshot) -> Sequence[MetaProposal]:
        ...
        return [
            MetaProposal(
                ...
                telemetry={
                    "target_probability": float(target[next_idx]),
                    "transition_probability": float(transition[current_idx, next_idx]),
                },
            )
        ]
...
class DynamicLiveSetParticleSkeleton:
    ...
    def weights(self, snapshot: RootSnapshot) -> dict[int, float]:
        ...
        for action in snapshot.actions:
            ...
            raw[action.action_id] = (
                self.competition_weight * competition
                + self.uncertainty_weight * uncertainty
            )
            if not action.visible:
                raw[action.action_id] += self.resurrection_fraction * max(
                    action.prior_anchor, 0.0
                )
    ...
    def propose(self, snapshot: RootSnapshot) -> Sequence[MetaProposal]:
        ...
        if not weights:
            return []
        allocations = {
            action_id: max(self.min_per_active, int(round(self.total_batch * weight)))
            for action_id, weight in weights.items()
            if weight > 0.0
        }
        ranked = sorted(
            allocations,
            key=lambda action_id: (weights[action_id], action_id),
            reverse=True,
        )
...
class TacticalSentinelSkeleton:
    ...
    def propose(self, snapshot: RootSnapshot) -> Sequence[MetaProposal]:
        forced = [
            ...
            if any(
                flag in {"forced_win", "forced_block", "mate", "immediate_loss_guard"}
                for flag in a.tactical_flags
            )
        ]
        if not forced:
            return []
        forced.sort(
            key=lambda a: (
                "forced_win" in a.tactical_flags or "mate" in a.tactical_flags,
                a.upper,
            ),
            reverse=True,
        )
