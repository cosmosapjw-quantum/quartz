# QUARTZ Idea Foundry — 24축 레거시 카탈로그

> **DEPRECATED / NON-CONTROLLING SNAPSHOT.** 이 문서는 원격 브랜치의 과거
> 24축 설계를 재현하기 위해 보존한다. 현재 단일 기준은 상위 디렉터리의
> A01--A26 문서와 `configs/idea_foundry.axes.v1.json`이다. 이 스냅샷의
> skeleton 또는 smoke test를 현재 축의 구현·효능·승격 근거로 사용하지 않는다.
...
Snapshot-local machine-readable record:

- `quartz/idea_foundry/legacy_24axis/registry.py`
- `configs/legacy/idea_foundry.axes.24axis.v1.json`

공통 Python 계약:

- `quartz/idea_foundry/legacy_24axis/contracts.py`

Rust runtime skeleton:

- `src/mcts/foundry/legacy_24axis/foundry_contracts.rs`
- `src/mcts/foundry/legacy_24axis/foundry_axes.rs`
