...
class RootSnapshot:
    ...
    def validate(self) -> None:
        ...
        if any(
            a.visits < 0 or a.pending < 0 or a.virtual_visits < 0 for a in self.actions
        ):
    ...
    def best_action(self) -> ActionEvidence | None:
        ...
        return max(
            visible, key=lambda a: (a.mean_q, a.visits, -a.edge_pos), default=None
        )
    ...
    def best_lower_action(self) -> ActionEvidence | None:
        visible = self.visible_actions
        return max(
            visible, key=lambda a: (a.lower, a.visits, -a.edge_pos), default=None
        )
