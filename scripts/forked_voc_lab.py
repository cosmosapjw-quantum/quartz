    ...
    for path in sorted(glob.glob(str(Path(trace_dir) / "*.json"))):
        ...
        if "trace_policies" in data and "trace_budgets" in data:
            bundles.append(
                {
                    "__path": path,
                    "trace_budgets": data["trace_budgets"],
                    "trace_policies": data["trace_policies"],
                }
            )
    return bundles
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Label frozen phase15 traces by realized value of computation"
    )
    p.add_argument(
        "--trace-dir", required=True, help="phase15 trace_cache dir (bundles as *.json)"
    )
    p.add_argument(
        "--strong-trace-dir",
        default=None,
        help="optional second (stronger) checkpoint's traces for a discrimination preview",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    per_bundle = [
        {
            "path": Path(b["__path"]).name,
            **{k: v for k, v in lab.label_trace_bundle(b).items() if k != "steps"},
        }
        for b in bundles
    ]
    ...
    resolved_config = {
        ...
        "strong_trace_dir": str(args.strong_trace_dir)
        if args.strong_trace_dir
        else None,
    ...
    manifest = build_run_manifest(
        ...
        source_paths=[
            ...
            REPO_ROOT / "scripts/forked_voc_lab.py",
        ],
        argv=sys.argv,
    ...
    manifest = finalize_run_manifest(
        manifest, output_dir=out, artifact_paths=[summary_path]
    )
