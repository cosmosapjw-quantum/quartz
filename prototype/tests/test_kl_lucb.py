...
def test_kl_lucb_gap_tight_does_not_fire():
    ...
    With min_pulls=30 only arms 0 and 1 participate; gap < 0."""
...
def test_kl_lucb_gap_wide_fires():
    """Audit-aligned wide-gap test: huge separation ⇒ gap > 0."""
...
def test_kl_lucb_gap_below_min_pulls_is_invalid():
    """Fewer than 2 arms with min_pulls ⇒ runner == best (caller's signal)."""
    mu_hats = [0.8, 0.5, 0.5]
    n_pulls = [100, 5, 5]
    gap, best, runner = kl_lucb_gap_bits(
