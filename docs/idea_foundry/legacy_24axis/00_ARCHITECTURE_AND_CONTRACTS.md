...
`src/mcts/foundry/legacy_24axis/foundry_axes.rs`의 `ShadowAxisPolicy<A>`는 foundry axis를 기존 `SearchPolicy::observe`에 연결하되 selection을 바꾸지 않는 adapter다.

## 3. Python 계약

`quartz/idea_foundry/legacy_24axis/contracts.py`의 핵심 타입:
...
```text
quartz/idea_foundry/legacy_24axis/
  contracts.py
  decision.py
  candidates.py
  systems.py
